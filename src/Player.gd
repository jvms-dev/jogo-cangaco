...
func fire_bullet():
	...
	"""
	if entrada_movimento.x != 0 and entrada_movimento.z == 0:
		var forward = arfagem.global_transform.origin.direction_to(global_transform.origin).normalized()
		forward.y = 0.0
		var right = Vector3.UP.cross(forward)
		var up = forward.cross(right)
		arfagem.global_transform.basis = Basis(right, up, forward).orthonormalized()"""
